  ...
  return (
    <div>
      ...
      <Dropdown>
        ...
        <Dropdown.Menu >
          {category && category.map((cate:any) => {
            return (
              <>
        ...
        <Row className='ab'>
          <Col sm={8}>
            <CardGroup className='card-groud'>
              {product && product.map((productitem:any) => <div className='product-container'>
            <Card >
            <Card.Img variant="top" src="https://wbc.net.au/wp-content/uploads/2021/05/covid-vaccine_title-page.png" />
                <Card.Body>
                  <Card.Title>
                    <a href={`/vaccine/${productitem.id}`}>{productitem.name}</a>
                  </Card.Title>
                  <Card.Text>
                    <span>Phòng bệnh: </span>
                    <p>{productitem.description}
                    </p>
